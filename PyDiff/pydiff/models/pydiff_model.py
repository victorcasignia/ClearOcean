...
class PyDiffModel(BaseModel):
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            # to_save_name = osp.splitext(val_data['lq_path'][0])[0].split('/')
            # img_name = f'{to_save_name[-3]}_{to_save_name[-1]}'
            ...
            # lq_path = osp.join(self.opt['path']['visualization'], f'lq_{img_name}_{idx}.png')
            # imwrite(lq_img, lq_path)
            # gt_path = osp.join(self.opt['path']['visualization'], f'gt_{img_name}_{idx}.png')
                ...
                if idx < self.opt['val'].get('show_num', 3) or self.opt['val'].get('show_all', False):
                    ...
                    if not self.opt['val'].get('ret_process', False):
                        if self.opt['val'].get('only_save_sr', False):
                            my_basename = osp.splitext(osp.basename(path))[0]
