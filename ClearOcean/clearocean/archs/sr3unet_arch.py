...
from clearocean.archs.layers import CALayer, SHALayer, SHALayerV2, CALayerV2
...
class Block(nn.Module):
    def __init__(self, dim, dim_out, groups=32, dropout=0, activation = 'swish'):
        super().__init__()

        if activation == 'swish':
            act_function = Swish()
        elif activation == 'swiglu':
            act_function = SwiGLU(dim)
        elif activation == 'glu':
            act_function = nn.GLU()
        else:
            act_function = Swish()
...
class SR3UNet(nn.Module):
    def __init__(
        ...
        if activation == 'swish':
            ...
            act_function = SwiGLU(inner_channel * 4)
        elif activation == 'glu':
            act_function = nn.GLU()
        else:
            act_function = Swish()
